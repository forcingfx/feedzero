...
Feature: Content view toggle
  ...
  Scenario: Extracted content matches feed
...
2. `#getAvailableModes()` computes which buttons to show:
   ...
   - "summary" — only if summary exists and differs from feed content (first 200 chars)
   - "extracted" — only if valid HTTP link exists and (not yet cached, or cached and differs from feed)
...
5. `#contentsSimilar(a, b)` strips HTML, normalizes whitespace, compares first 200 lowercase chars
...
| File | Coverage |
|------|----------|
| `tests/ui/components/article-view.test.js` | 9 tests: empty state, content render, toggle hidden (no summary/no link), summary hidden (similar), summary shown (different), timestamp with time, XSS escape, fallback content, reset |
...
- **Smart mode hiding** — Redundant modes are hidden to avoid confusing the user with identical content under different labels. The similarity heuristic compares the first 200 characters of stripped text.
...
- Similarity heuristic uses first 200 chars — edge cases with very similar but different articles
