...
- **Shared rate-limit cooldown between prefetch and on-demand extraction** — `/api/page` is metered by the same 300-req/60s bucket that `/api/feed` uses, and a refresh-time prefetch burst (up to 20 articles per opted-in or "frequent" feed) can exhaust it. The first 429 anyone observes — prefetch worker or manual "Full text" click — calls `signalRateLimited(retryAfter)` on `src/core/extractor/prefetch-cooldown.ts`, which (1) tells the worker pool in `prefetch-service.ts` to abandon the rest of the batch and skip future passes for the window, and (2) records a `rateLimitedMap` entry in `extraction-store` that the reader pane renders as `<RateLimitedNotice>` (visible countdown, "Try again" enabled when the window elapses, "Open original" always available). Replaces the silent disabled-toggle behavior that produced reports like the 2026-05-25 "extraction seems broken" thread.
