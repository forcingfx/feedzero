...
- **Resolved at:** 2026-05-23 ~17:25 UTC (commit `70e8d92` pushed
  to PR #180)
...
Commit `70e8d92`, 21 insertions / 10 deletions.

## Prevention

Tracked separately as follow-ups; not all of these landed in the
fix commit.

- **Production-bundle boot smoke test.** Add
  `tests/smoke/boot.test.ts` that, when `SMOKE_TESTS=1`, hits a
  configurable `SMOKE_BASE_URL` (default `https://my.feedzero.app`)
  with headless chromium and asserts the page reaches a known
  DOM state ("Open command palette" button visible) within N
  seconds. Equivalent to the headless repro I ran in this session;
  cheap to run post-deploy. CI gate: run on every push to main
  after the Vercel deploy hook completes.
- **Local production-bundle boot test in pre-merge CI.** Wrap the
  same headless-chromium boot against `npm run build && npm run
  serve` in a non-smoke-gated Vitest test. Catches this class of
  Rollup-chunk-graph regression before the deploy ever happens.
- **Lint / build rule for dynamic imports of in-tree modules.**
  When a relative `import("./foo.ts")` is followed by a destructure
  of a named export and `foo.ts` is part of the same bundle root,
  refuse the build. The pattern is almost never what you want —
  static imports give you the same code-splitting via Vite's
  per-chunk inference, with correct property resolution.
- **Treat `INEFFECTIVE_DYNAMIC_IMPORT` warnings as build errors.**
  The warning was a true positive every build, ignored every
  build. Either fix the underlying imports or explicitly suppress
  the specific cases that are intentional. The current state —
  warning present, build passes, prod broken — is the worst of
  both worlds.
- **CLAUDE.md addition.** A new principle in the "Key Patterns"
  section: *"Prefer static imports of in-tree modules at file top,
  even across module cycles. Cycles where both modules only touch
  each other's exports at runtime (inside function bodies) are
  safe in JavaScript. Dynamic `import('./foo.ts')` followed by
  named-export destructure is brittle — Rollup's chunk-graph
  decisions can move the target into a parent chunk that doesn't
  expose source-name properties, and the destructure silently
  returns `undefined`."*
...
- PR [#178 — test-only adapter
  branding](https://github.com/forcingfx/feedzero/pull/178). Not
  the proximate cause (bisect confirmed bug pre-dates merge), but
  the chunk-boundary shift it introduced likely tipped the
  manifestation threshold. The investigation hypothesis that #178
  was the cause was disproved by reverting it locally and seeing
  the crash persist.
- PR [#158 — UserPreferences type and
  constants](https://github.com/forcingfx/feedzero/pull/158).
  Introduced `preferences-store.ts` with the dynamic-import +
  destructure pattern. Latent since this merge; manifestation
  depended on whichever chunk Rollup placed `app-store.ts` into,
  which has shifted multiple times since.
- Incidents
