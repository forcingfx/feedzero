...
- **Canonical tier matrix**: `src/core/features/tier-matrix.ts` is the single source of truth for which features exist at which tier and with what scope/limit. `feature-gates.ts` (binary capability) and `quotas.ts` (continuous limit, e.g. `FREE_FEED_LIMIT`) both derive from it — never hand-edit those for tier changes; edit the matrix. The human-readable view at `docs/tier-matrix.md` regenerates via `npm run docs:tier-matrix` (`-- --check` is the staleness guard).
