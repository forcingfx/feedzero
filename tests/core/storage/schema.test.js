...
import { createFeed, createArticle, validateFeed, validateArticle, SCHEMA_VERSION } from '../../../src/core/storage/schema.js';
...
describe('Schema', () => {
  ...
  describe('createFeed', () => {
    it('should create a feed with required fields', () => {
      const result = createFeed({ url: 'https://example.com/rss', title: 'Example' });
  ...
  describe('createArticle', () => {
    it('should create an article with required fields', () => {
      const result = createArticle({ feedId: 'f1', title: 'Post', link: 'https://x.com/1' });
      expect(isOk(result)).toBe(true);
    ...
    it('should reject missing feedId', () => {
      expect(isErr(createArticle({ title: 'X', link: 'https://x.com' }))).toBe(true);
    });
  });
  ...
  describe('validateArticle', () => {
    it('should accept valid article', () => {
      const article = unwrap(createArticle({ feedId: 'f1', title: 'P', link: 'https://x.com' }));
      expect(isOk(validateArticle(article))).toBe(true);
    });

    it('should reject article missing link', () => {
      expect(isErr(validateArticle({ id: '1', feedId: 'f1', title: 'P' }))).toBe(true);
    });
  });
});
